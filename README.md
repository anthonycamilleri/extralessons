...
- Read the terms and conditions, and write to the office through the contact form at `/contact/` — no account needed. Messages go to the contact address in Site configuration (`info@esljparents.eu` by default) through the notification outbox, with the writer's address as Reply-To, so the office just hits reply. A hidden honeypot field and an hourly per-caller cap keep the drive-by spam out.
...
caching; an S3 bucket is a one-variable switch), email over SMTP through
Scaleway Transactional Email, applied and test-sent by every deploy.
...
| `migrate` | Serverless Job `extralessons-migrate`, once per deploy | `manage.py migrate --noinput` (and, once per estate, `manage.py ensure_admin`) |
...
The estate was on Render for a while in between; how it came back is recorded
in [docs/migration-render-to-scaleway.md](docs/migration-render-to-scaleway.md).
The Render pipeline and the migration workflows were removed once the move was
done.
...
| `EMAIL_BACKEND` | Production sets `django.core.mail.backends.smtp.EmailBackend` explicitly (deploy/scaleway-env.lib.sh). Unset: ZeptoMail API when its token is set, SMTP otherwise |
| `EMAIL_HOST` / `EMAIL_PORT` / `EMAIL_USE_TLS` | Scaleway Transactional Email: `smtp.tem.scaleway.com`, `587`, `true` (STARTTLS). Scaleway blocks outbound 25 and 465 to other hosts |
| `EMAIL_HOST_USER` / `EMAIL_HOST_PASSWORD` | The project id the sending domain is registered in, and an API secret key allowed to send (`SCW_MAIL_SECRET_KEY` in GitHub) |
| `ZEPTOMAIL_SEND_MAIL_TOKEN` | Alternative: Zoho ZeptoMail Mail Agent token for `apps/notifications/backends/zeptomail.py` |
...
| `DEFAULT_FROM_EMAIL` | From address, e.g. `School Activities <notifications@example.com>`. Must be on a domain verified with the mail provider |
...
| `ADMIN_EMAIL` | First administrator. `manage.py ensure_admin` (run once by `deploy/provision.sh`) creates the superuser and emails a set-password link; `manage.py send_test_email` (run by every deploy) sends its email check here |
...
- **Site configuration** (singleton): school name, the sender name that signs every email (default "European School PTA"), contact email (the address the contact form delivers to and the footer shows; empty hides both), catalogue intro text, whether parent self-signup is open, waiting-list offer expiry in hours, toggles for the admin alert emails (new request, seat freed), and the terms and conditions in Markdown (served at `/terms/`, linked from the navigation and footer, and required as a tick on the registration form; empty hides all three).
...
(`manage.py migrate`) and waits for it, sends a test email from the estate,
points the notifier job at the new image, rolls the container, smoke-tests
`/_health`, and finally prunes the registry to the ten newest images
(`deploy/scw-prune-images.sh`; `latest` and the image just deployed always
survive).
...
that image. Only the ten newest images are kept, so further back means a
revert and a fresh build.

Every deploy is a cold start of the whole estate: job runs, a new container
instance kept alive for the idle window, the database woken. It is the most
expensive thing the estate does when nobody is using it, so merge a batch of
commits as one push rather than pushing them one by one.
...
configure the estate* workflow, which is also where the container's
`min-scale` is changed from GitHub. The other operator workflows under
*Actions* are *Inspect hosting estate* (read-only; prints the container's
current scale and the database's `cpu-min`) and *Scaleway: attach domains*.
...
config/
  ...
  robots.py           # /robots.txt: crawlers get the catalogue, not the logins, API and helpers
...
deploy/
  ...
  scw-prune-images.sh # delete old image tags from the registry, keeping latest, the deployed one and N newest
  scw-merge-env.sh    # re-send a resource's plain environment with changes merged in
  ...
  pre-deploy.sh       # migrate + ensure_admin as one command; CI runs it inside the built image
docs/
  scaleway-setup.md   # one-time setup and operations on Scaleway Serverless
  migration-render-to-scaleway.md  # how production came back from Render (history; its workflows are gone)
apps/
  ...
  notifications/      # NotificationTemplate, Broadcast, Notification (outbox rows);
                      ...
                      # send_test_email.py (the deploy's proof that mail goes out)
...
tests/                # pytest suite (services, capacity race, notifications, views,
                      # school holidays, health probe, robots.txt, inline delivery, notifier
                      # job modes, MCP tools)
.mcp.json             # Claude Code picks up the MCP server from here
.github/workflows/
  ci.yml              # tests on SQLite + Postgres, deploy checks, image build
  deploy.yml          # after CI on main: build → migrate job (+ email settings, test email) → notifier job → container → smoke test → prune registry
  scaleway-configure.yml, scaleway-domains.yml, estate-inspect.yml
                      # operator workflows: environment and min-scale, TLS domains, read-only inspection
```
