...
| `ADMIN_EMAIL` | First administrator. `manage.py ensure_admin` (run by the pre-deploy step) creates the superuser and emails a set-password link; `manage.py send_test_email` (run by every deploy) sends its email check here |
...
.github/workflows/
  ...
  deploy.yml          # after CI on main: build → migrate job (+ email settings, test email) → notifier job → container → smoke test
