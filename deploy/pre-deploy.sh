...
#
# On Scaleway the deploy workflow runs migrate as the migrate job and
# provision.sh runs ensure_admin once (deploy.yml, provision.sh), so nothing
# there calls this file; it exists for platforms with a single pre-deploy
# command, which run it without a shell — hence a script rather than
# "a && b". CI runs it inside the built image (see ci.yml), so it stays proven
# either way.
#
