...
| One 500 with `OperationalError: internal error` on the first request after a quiet spell, then everything works | The database scaled to zero and the app's pool served a connection whose backend had gone | Fixed in `config/settings/prod.py` (the pool checks connections on checkout). If it recurs, make sure the deployed image carries that change; `cpu-min=1` on the database removes cold starts entirely |
